...
ParserProc :: proc(parser: ^TreeParser, prev_node: ^ASTNode) -> (token: Token, op_type: OpType, precedence: int, right_associative: bool)
ASTNode :: struct {
  using token: Token,
  left, right: ^ASTNode,
}
...
@(private = "file")
_parser_eat_token :: #force_inline proc(parser: ^TreeParser, token: Token) {
  parser.start += len(token.slice)
  parser.keep_going = parser.start < len(parser.str)
}
// TODO: add `file:line:column` prefix to the error
@(cold)
report_parser_error :: proc(parser: ^TreeParser, error: string) {
  parser.keep_going = false
  if len(parser.error) == 0 {parser.error = error}
}
@(private = "file")
_parse_recursively :: proc(parser: ^TreeParser, min_precedence: int, allocator := context.temp_allocator) -> (prev_node: ^ASTNode) {
